...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								...
								'group_id'	=> $user->group_id, //**** ADDED
								'notify_user'	=> $user->notify_user, //**** ADDED
								'banned' => $user->banned //*** ADDED
						));
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'status' => '', 'email' => '', 'group_id' => '', 'notify_user' => '')); //***CLEAR ALL CRUCIAL SETTINGS THAT WE STORED DURING LOGIN; MOST CRUCIAL ONE IS id
		//$this->ci->session->sess_destroy(); //I COMMENTED THIS LINE BECAUSE IT THROWS FOLLOWING ERRORS WHEN LOGOUT IS CLICKED: Undefined index: session_id, ip_address, user_agent, last_activity and session_id. They come from Filename: libraries/Session.php. Such problem didn't happen with CentBoox
	}
	 ...
	 * @return	string
	 */
	function get_notify_user() //**** ADDED
	{
		return $this->ci->session->userdata('notify_user');
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation, $attachment_folder = FALSE) //*** ADDED: $attachment_folder = FALSE. It should be true if we want to create a website that allows upload/download attachments. 
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			if (!is_null($res = $this->ci->users->create_user($data, !$email_activation))) {
				...
				if($attachment_folder)//*** ADDED
				{
					mkdir(ATTACHMENTS_FOLDER.'/'.$res['user_id']); //*** create a folder for the user. User's attachments will be uploaded to that user's particular folder.
					mkdir(ATTACHMENTS_FOLDER.'/'.$res['user_id'].'/thumbs'); //*** create thumbs folder for that particular user
				}
