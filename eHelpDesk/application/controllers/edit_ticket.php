...
//allow members to edit a post.
class Edit_ticket extends Secured_Frontend_Controller {
	...
	{	
		...
		$data['post'] = $this->v_ticket_model->read('ticket_id, title, message, attachment, category_id, category_name, priority_id, priority_name, status_id, status_name, date_created, date_closed, date_updated, username', "ticket_id = $post_id AND is_ticket_active = '1' AND users_id = ".$this->tank_auth->get_user_id()); //only show posts that are active and belongs to the user	
		...
		//Is the ticket in a closed state? or Did any member, other than post owner, replied to post? If yes, don't allow access to the page. NOTE: i selected any column, in my case i chose users_id, just not to put *. All i care in here, did i get records back or not!
		$data['is_stuff_replied'] = $this->v_reply_model->read('users_id', "ticket_id = $post_id AND is_active = '1' AND users_id <> ".$this->tank_auth->get_user_id());
		if($data['is_stuff_replied'] || $data['post']['rows'][0]->status_id == '2')
		{
			redirect("ticket/$post_id"); //you can show the following message (optional): you can not delete or edit the ticket because support stuff already reponsed to your ticket.
		}
	...
	{	
		...
		if(isset($_FILES['edit_ticket_attachment'])) //if the attachment box is showing, do the following:
		{
			...
			$data['effected_rows'] = $this->ticket_model->update("ticket_id = $p_id", array('title' => $this->input->post('edit_ticket_title', TRUE), 'message' => $this->input->post('edit_ticket_message', TRUE), 'attachment' => $attachment_file_name, 'users_id' => $this->tank_auth->get_user_id(), 'priority_id' => $this->input->post('priority_lookup'), 'category_id' => $this->input->post('category_lookup')));
		}
